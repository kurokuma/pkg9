...
This script is intentionally download-only. It does not unpack archives,
install dependencies, or execute any package lifecycle hooks.
"""
...
def main() -> int:
    ...
    for spec in specs:
        try:
            ...
            dest = out_dir / tarball_name(name, version)
            if dest.exists() and not args.overwrite:
                print(f"SKIP {spec} -> {dest.name}")
                skipped += 1
                continue
            download_file(tarball_url, dest, args.timeout)
            print(f"OK   {spec} -> {dest.name}")
            success += 1
